...
const Register = () => {
    ...
    const isSocialLogin = useMemo(
        () => user?.providerData?.some((p) => p.providerId === 'google.com' || p.providerId === 'apple.com') ?? false,
        [user]
    );
    ...
    const handleFinalizeSocial = async () => {
        ...
        try {
            Alert.alert('In arrivo', 'Il completamento profilo sul server verrà collegato qui (registrazione social).');
        } finally {
    ...
    const handleNext = () => {
        ...
        if (step === 2 && isSocialLogin) void handleFinalizeSocial();
        else if (step < TOTAL_STEPS - 1) setStep((s) => s + 1);
    };
        ...
        <View style={{ flex: 1, backgroundColor: pageBg }}>
            <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
                <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
                    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ flexGrow: 1, paddingHorizontal: 24, paddingTop: insets.top + 20, paddingBottom: insets.bottom + 24 }} showsVerticalScrollIndicator={false}>
                        <View style={{ width: '100%', maxWidth: 400, alignSelf: 'center' }} collapsable={false}>
                                ...
                                <>
                                    <View style={{ marginBottom: 18 }}>
                                        ...
                                        <View style={fieldRowStyle}>
                                            ...
                                            <TextInput value={name} onChangeText={setName} style={inputStyle} placeholder="Nome" placeholderTextColor={muted} autoCapitalize="words" textContentType="givenName" />
                                        </View>
                                    </View>
                                    <View style={{ marginBottom: 18 }}>
                                        ...
                                        <View style={fieldRowStyle}>
                                            <ThemedIcon name="person-outline" size={20} lightColor={muted} darkColor={muted} />
                                            <TextInput value={lastName} onChangeText={setLastName} style={inputStyle} placeholder="Cognome" placeholderTextColor={muted} autoCapitalize="words" textContentType="familyName" />
                                        </View>
                                    </View>
                                ...
                                <>
                                    <View style={{ marginBottom: 18 }}>
                                        ...
                                        <View style={fieldRowStyle}>
                                            ...
                                            <TextInput value={username} onChangeText={setUsername} style={inputStyle} placeholder="username" placeholderTextColor={muted} autoCapitalize="none" autoCorrect={false} textContentType="username" />
                                        </View>
                                    </View>
                            ...
                            {step === 2 && (
                                <>
                                    ...
                                    <BouncyPressable disabled={!isValidIsoDate(birthDate.trim()) || isSubmitting} isLoading={isSocialLogin && isSubmitting} loadingColor={ctaFg} onPress={handleNext} style={ctaStyle}>
                                        <ThemedText style={btnLabelStyle} lightColor={ctaFg} darkColor={ctaFg}>
                                            {isSocialLogin ? (isSubmitting ? 'Completamento...' : 'Completa registrazione') : 'Avanti'}
                                        </ThemedText>
                                    </BouncyPressable>
                                </>
                            )}
                                ...
                                <>
                                    <View style={{ marginBottom: 18 }}>
                                        ...
                                        <View style={fieldRowStyle}>
                                            ...
                                            <TextInput value={email} onChangeText={setEmail} style={inputStyle} placeholder="nome@email.com" placeholderTextColor={muted} keyboardType="email-address" autoCapitalize="none" autoCorrect={false} textContentType="emailAddress" />
                                        </View>
                                    </View>
                                    <View style={{ marginBottom: 18 }}>
                                        ...
                                        <View style={fieldRowStyle}>
                                            ...
                                            <TextInput value={password} onChangeText={setPassword} style={inputStyle} placeholder="••••••••" placeholderTextColor={muted} secureTextEntry={!showPassword} textContentType="newPassword" />
                                    ...
                                    <View style={{ marginBottom: 24 }}>
                                        ...
                                        <View style={fieldRowStyle}>
                                            <ThemedIcon name="lock-closed-outline" size={20} lightColor={muted} darkColor={muted} />
                                            <TextInput value={confirmPassword} onChangeText={setConfirmPassword} style={inputStyle} placeholder="••••••••" placeholderTextColor={muted} secureTextEntry={!showPassword} textContentType="newPassword" />
                                        </View>
                                    </View>
                                    <BouncyPressable disabled={email.trim() === '' || password.trim() === '' || confirmPassword.trim() !== password.trim() || password.length < 6} style={ctaStyle}>
